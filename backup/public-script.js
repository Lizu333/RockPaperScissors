...
const choiceNames = {
    hu: { rock: "KŐ", paper: "PAPÍR", scissors: "OLLÓ" },
...
const translations = {
    en: {
        ...
        endWinTitle: "YOU WIN!",
        endWinMessage: "Congratulations, you reached the target score first!",
        endLossTitle: "YOU LOST!",
        endLossMessage: "The opponent reached the target score first."
    },
    hu: {
        title: "Kő - Papír - Olló",
        ...
        endWinTitle: "NYERTÉL!",
        endWinMessage: "Gratulálunk, te érted el előbb a célpontszámot!",
        endLossTitle: "VESZTETTÉL!",
        endLossMessage: "Az ellenfél érte el előbb a célpontszámot."
    }
};
...
function emptyStatistics() {
    return { gamesPlayed: 0, wins: 0, losses: 0, draws: 0, rock: 0, paper: 0, scissors: 0 };
}
...
async function startGame() {
    if (gameStarting || roundInProgress) return;
...
async function playRound(playerChoice) {
    if (!currentUser || gameFinished || roundInProgress) return;
    ...
    resultElement.textContent = translations[currentLanguage].roundStarted;
    playerChoiceName.textContent = choiceNames[currentLanguage][playerChoice] || playerChoice.toUpperCase();
    setChoiceArea("player-choice", playerChoice);

    computerChoiceElement.textContent = "?";
    computerChoiceName.textContent = translations[currentLanguage].computerChoosing;
    ...
    try {
        ...
        const choicesKeys = ["rock", "paper", "scissors"];
        for (let i = 0; i < 6; i++) {
            const randomChoice = choicesKeys[i % 3];
            setChoiceArea("computer-choice", randomChoice);
            playSound("computer");
            await sleep(120);
        }
        ...
        if (!response.ok) {
            console.error(data.error || "A kör indítása sikertelen.");
            if (response.status === 401) {
                currentUser = null;
                updateProfileUI();
            }
        ...
        setChoiceArea("player-choice", data.playerChoice);
        playerChoiceName.textContent = choiceNames[currentLanguage][data.playerChoice];
        playerChoiceElement.classList.add("choice-reveal");

        setChoiceArea("computer-choice", data.computerChoice);
        computerChoiceName.textContent = choiceNames[currentLanguage][data.computerChoice];
        ...
        let resultText;
        if (data.result === "draw") {
            resultText = translations[currentLanguage].roundDraw;
            playSound("draw");
        } else if (data.result === "win") {
            resultText = translations[currentLanguage].roundWin;
            playSound("win");
        } else {
            resultText = translations[currentLanguage].roundLoss;
            playSound("lose");
        }
        ...
        await sleep(800);
        resultElement.textContent = translations[currentLanguage].chooseWeapon;
        setChoiceButtonsDisabled(false);
    } catch (error) {
        console.error(error);
        setChoiceButtonsDisabled(false);
...
async function finishGame(finalResultFromRound = null) {
    ...
    const finalResult = finalResultFromRound || (playerScore > computerScore ? "win" : "loss");
    ...
    const winMsg = currentLanguage === "hu" 
        ? `Gratulálunk, te érted el előbb a ${targetScore} pontot!` 
        : `Congratulations, you reached ${targetScore} points first!`;
        
    const lossMsg = currentLanguage === "hu" 
        ? `Az ellenfél érte el előbb a ${targetScore} pontot.` 
        : `The opponent reached ${targetScore} points first.`;

    if (finalResult === "win") {
        document.getElementById("end-title").textContent = translations[currentLanguage].endWinTitle;
        document.getElementById("end-message").textContent = winMsg;
        playSound("gameover-win");
    } else {
        document.getElementById("end-title").textContent = translations[currentLanguage].endLossTitle;
        document.getElementById("end-message").textContent = lossMsg;
        playSound("lose");
    }
...
async function registerUser(username, password) {
    ...
    const gender = genderElement ? genderElement.value : "girl";
    ...
    try {
        ...
        console.error(error);
        errorElement.textContent = currentLanguage === "hu" ? "Nem sikerült kapcsolódni a szerverhez." : "Could not connect to the server.";
    }
}
...
async function loginUser(username, password) {
    ...
    try {
        ...
        const data = await response.json();
        if (!response.ok) {
            errorElement.textContent = data.error || (currentLanguage === "hu" ? "Hibás felhasználónév vagy jelszó!" : "Invalid username or password!");
            return;
        }
...
function updateProfileUI() {
    ...
    if (currentUser) {
        ...
        joinDateElem.textContent = currentUser.joined || "-";
        updateStatisticsUI();
    } else {
...
async function setTheme(themeKey, saveToServer = true) {
    ...
    if (!saveToServer || !currentUser) return;
...
function setLanguage(lang) {
    ...
    if (soundToggleText) {
        soundToggleText.textContent = soundEnabled ? translations[currentLanguage].on : translations[currentLanguage].off;
    }
...
function updateStatisticsUI() {
    ...
    if (gamesPlayed) gamesPlayed.textContent = statistics.gamesPlayed;
    if (totalWins) totalWins.textContent = statistics.wins;
    if (totalLosses) totalLosses.textContent = statistics.losses;
    if (totalDraws) totalDraws.textContent = statistics.draws;
    if (rockUsed) rockUsed.textContent = statistics.rock;
    if (paperUsed) paperUsed.textContent = statistics.paper;
    if (scissorsUsed) scissorsUsed.textContent = statistics.scissors;

    const winRate = statistics.gamesPlayed > 0 ? Math.round((statistics.wins / statistics.gamesPlayed) * 100) : 0;
    if (winRateElement) winRateElement.textContent = `${winRate}%`;
}

document.querySelectorAll("button").forEach(btn => {
    btn.addEventListener("click", () => {
        if (btn.id !== "sound-toggle") playSound("click");
    });
});

document.querySelectorAll(".target-score-btn").forEach(btn => {
    btn.addEventListener("click", () => {
        document.querySelectorAll(".target-score-btn").forEach(b => b.classList.remove("active"));
        btn.classList.add("active");
        targetScore = parseInt(btn.dataset.score, 10);
    });
});

const langModalBtn = document.getElementById("lang-modal-btn");
if (langModalBtn) langModalBtn.addEventListener("click", () => openModal("settings-modal"));

const themeModalBtn = document.getElementById("theme-modal-btn");
if (themeModalBtn) themeModalBtn.addEventListener("click", () => openModal("settings-modal"));

const startBtn = document.getElementById("start-btn");
if (startBtn) startBtn.addEventListener("click", startGame);

const profileBtn = document.getElementById("profile-btn");
if (profileBtn) profileBtn.addEventListener("click", () => { updateProfileUI(); openModal("profile-modal"); });
...
if (menuGameBtn) menuGameBtn.addEventListener("click", () => showScreen("menu-screen"));

const menuBtn = document.getElementById("menu-btn");
if (menuBtn) menuBtn.addEventListener("click", () => showScreen("menu-screen"));
...
document.querySelectorAll(".choice-btn").forEach(btn => {
    btn.addEventListener("click", () => {
        if (btn.disabled || roundInProgress || gameFinished) return;
...
if (soundToggle) {
    soundToggle.addEventListener("click", () => {
        ...
        document.getElementById("sound-toggle-text").textContent = soundEnabled ? translations[currentLanguage].on : translations[currentLanguage].off;
    });
}
...
const resetStatisticsBtn = document.getElementById("reset-statistics-btn");
if (resetStatisticsBtn) {
    resetStatisticsBtn.addEventListener("click", async () => {
        if (!currentUser) return;
        try {
...
if (reportModalBtn) reportModalBtn.addEventListener("click", () => openModal("report-modal"));

const privacyModalBtn = document.getElementById("privacy-modal-btn");
if (privacyModalBtn) privacyModalBtn.addEventListener("click", () => openModal("privacy-modal"));

const savedTheme = localStorage.getItem("gameTheme") || "pink-brown";
const savedLanguage = localStorage.getItem("gameLanguage") || "hu";
